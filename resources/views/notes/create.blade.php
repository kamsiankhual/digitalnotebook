...
@section('content')
    <div class="max-w-4xl mx-auto">
        ...
        <div class="mb-5">
            ...
            <h1 class="text-3xl font-bold text-gray-900 mt-3">Create New Note</h1>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-8">
            ...
            <form action="{{ route('notes.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        @error('title')
                            <span class="text-red-600 text-xs">{{ $message }}</span>
                        @enderror
                        <input type="text" name="title" class="w-full border-2 border-gray-300 rounded-lg focus:outline-none px-2 py-2 text-xs transition-colors" placeholder="e.g.Authentication Setup" value="{{ old('title') }}">
                    </div>
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Description</label>
                    <textarea name="description" id="description" rows="10" class="w-full rounded-lg border-2 border-gray-300 focus:outline-none px-2 py-1 text-xs" rows="10">{{ old('description') }}</textarea>
                </div>
                ...
                <div class="pt-5 border-t border-gray-100 flex justify-end gap-3">
                    ...
                    <button type="submit" class="py-2 px-6 border border-transparent shadow-sm text-sm font-medium rounded-lg text-white bg-indigo-600 hover:bg-indigo-700">Save Note</button>
                </div>
            </form>
        </div>
    </div>
